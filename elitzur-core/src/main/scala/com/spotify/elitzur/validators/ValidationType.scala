...
/**
  ...
  */
abstract class DynamicValidationType[T, U, A <: DynamicValidationType[
  T,
  U,
  A
]: ({ type L[x] = DynamicCompanionImplicit[T, U, x] })#L]
    extends BaseValidationType[T] {

  private[elitzur] def arg: Option[U]

  def setArg(a: U): A = {
    implicitly[DynamicCompanionImplicit[T, U, A]].companion
      .parseWithArg(data, a)
  }
}
