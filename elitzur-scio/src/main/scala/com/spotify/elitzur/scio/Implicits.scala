...
trait Implicits {
  ...
  implicit class SCollectionImplicitValidatorFns[T: Coder](sc: SCollection[T])(
    ...
    def validate(
        conf: ValidationRecordConfig = ValidationRecordConfig()
    ): SCollection[T] = {
      sc.withName("validate")
        .applyTransform(ParDo.of(new ValidatorDoFn(vr, config = conf)))
    }

    def validateWithResult(
        conf: ValidationRecordConfig = ValidationRecordConfig()
  ...
  implicit class SCollFromAvroConverter[GR <: GenericRecord: Coder](
      sc: SCollection[GR]
  ) {
    def fromAvro[T: Coder](implicit c: AvroConverter[T]): SCollection[T] = {
      sc.withName("fromAvro")
        .applyTransform(ParDo.of(new FromAvroConverterDoFn(c)))
    }
  }

  implicit class SCollToAvroConverter[T: AvroConverter: Coder](
      sc: SCollection[T]
  ) {
    def toAvro[GR <: GenericRecord: Coder: ClassTag](implicit
        c: AvroConverter[T]
    ): SCollection[GR] = {
  ...
  implicit class SCollToAvroDefaultConverter[T: AvroConverter: Coder](
      sc: SCollection[T]
  ) {
    def toAvroDefault[GR <: GenericRecord: Coder: ClassTag](
        defaultR: GR
    )(implicit c: AvroConverter[T]): SCollection[GR] = {
      sc.withName("toAvroDefault")
        .applyTransform(ParDo.of(new ToAvroDefaultConverterDoFn(defaultR, c)))
    }
  }
}
