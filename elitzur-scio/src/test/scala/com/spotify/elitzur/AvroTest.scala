...
object AvroTestCaseClasses {
  case class UnnestedValidated(
      countryCode: ValidationStatus[CountryCodeTesting],
      userId: String,
      playCount: Long
  )
  case class Inner(
      countryCode: CountryCodeTesting,
      userId: String,
  ...
  case class UnnestedNoValidation(
      countryCode: String,
      userId: String,
      playCount: Long
  )
  ...
  case class RepeatedInnerRecordCC(
      stringField: CountryCodeTesting,
      longField: NonNegativeLongTesting
  )
  case class RepeatedRecordCC(
      repeatedRecord: Seq[RepeatedInnerRecordCC],
...
object AvroArbs {
  ...
  implicit def validationStatusArb[T](implicit
      a: Arbitrary[T]
  ): Arbitrary[ValidationStatus[T]] = {
  ...
  object ArbInstances {
    ...
    val arbNestedValidated: Arbitrary[NestedValidated] =
      implicitly[Arbitrary[NestedValidated]]
...
class AvroTest extends PipelineSpec {
  ...
  "AvroCaseClassConverter" should "convert and then validate for a simple record" in {
    val testRecordsValid = Gen
      .listOfN(
        scLength,
        avroOf[InnerNestedType]
  ...
  "AvroCaseClassConverter" should "round trip Avro for a simple record" in {
    ...
    val testRecords = Gen
      .listOfN(
        ...
        Gen
          .const("US")
          .flatMap(s => Gen.posNum[Long].map(l => (l, s)))
          .flatMap {
            case (l, s) =>
              arbUnnested.arbitrary.map(
                _.copy(
                  countryCode = CountryCodeTesting(s),
  ...
  "AvroCaseClassConverter" should "round trip Avro for a repeated record" in {
    val testRecords = Gen
      .listOfN(
        ...
        for {
          cc <-
            Gen.oneOf("US", "SE", "CA", "UK", "GY").map(CountryCodeTesting(_))
    ...
    runWithContext { sc =>
      val r: SCollection[RepeatedRecordCC] = sc
        .parallelize(testRecords)
  ...
  "AvroCaseClassConverter" should "convert and validate for a simple record wrapped in " +
    ...
    runWithData(testRecordsValid)(sc =>
      sc.map(e =>
        implicitly[AvroConverter[UnnestedValidated]].fromAvro(e, e.getSchema)
      ).validate()
        .count
    ) shouldBe Seq(scLength)
  }
  ...
  "AvroCaseClassConverter" should "round trip Avro for a simple record with wrapped status" in {
    ...
    val testRecords = Gen
      .listOfN(
        ...
        Gen
          ...
          .flatMap {
            case (l, s) =>
              arbUnnestedValidated.arbitrary.map(
                _.copy(
    ...
    runWithContext { sc =>
      val r: SCollection[UnnestedValidated] = sc
        .parallelize(testRecords)
        .toAvro[InnerNestedType]
  ...
  "AvroCaseClassConverter" should "round trip for a simple nested record" in {
    val testRecordsInner =
      Gen.const("US").flatMap(s => Gen.posNum[Long].map(l => (l, s))).flatMap {
        case (l, s) =>
          arbUnnested.arbitrary.map(
            _.copy(
              countryCode = CountryCodeTesting(s),
    ...
    val testRecordsValid = Gen
      .listOfN(
        scLength,
        testRecordsInner.flatMap { i =>
          arbNested.arbitrary.map(n => n.copy(inner = i))
        }
      )
      .sample
      .get
  ...
  "AvroCaseClassConverter" should "round trip for a nested record with a wrapped field" in {
    ...
    val testRecordsValid = Gen
      .listOfN(
        scLength,
        testRecordsInner.flatMap { i =>
          arbNestedValidated.arbitrary.map(n => n.copy(inner = i))
        }
      )
      .sample
      .get
  ...
  "AvroCaseClassConverter" should "convert and then validate for a " +
    ...
    runWithData(testRecordsValid)(sc =>
      sc
        ...
        .validate()
        .map(
          _.userOptionalLong.get
        ) // check that Option was created successfully
  ...
  "AvroCaseClassConverter default" should "convert and then convert w/default for a case class" +
    ...
    val testRecordDefault =
      Gen.listOfN(scLength, avroOf[RepeatedRecord]).sample.get.headOption.get
    ...
    runWithData(testRecords)(sc =>
      ...
      testRecords
        .map(_ => testRecordDefault.getRepeatedRecord.get(0).getLongField)
        .toSeq
  }
  ...
  "AvroCaseClassConverter" should "fail to convert for a " +
    ...
    npe.getClass.getCanonicalName shouldBe classOf[
  ...
  "String Conversion" should "work with org.apache.avro.util.Utf8" in {
    val avroWithUtf8 = InnerNestedType
      .newBuilder()
  ...
  "toAvroDefault" should "work with nullable nested records" in {
    val testRecordDefault =
      NullableNestedRecord.newBuilder().setInner(null).build()
    val testRecord =
      avroOf[NullableNestedRecord].amend(Gen.const(null))(_.setInner).sample.get
    val c = implicitly[AvroConverter[NullableNested]]

    c.toAvroDefault(
      c.fromAvro(testRecord, NullableNestedRecord.SCHEMA$),
      testRecordDefault
    )
  }
  ...
  "AvroCaseClassConverter" should "convert and then validate for a dynamic record with arg" in {
    ...
    runWithData(Seq(testRecord))(sc =>
      ...
      Valid(DynamicRecord(DynamicString("String", Set("String"))))
    )
  }

  "AvroCaseClassConverter" should "round trip for record with an enum field" in {
    val testRecords = Gen.listOfN(10, avroOf[EnumType]).sample.get
